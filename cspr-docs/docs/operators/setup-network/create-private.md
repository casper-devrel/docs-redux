...
refund_handling = { type = "refund", refund_ratio = [1, 1] }
```
After deducting the gas fee, the distribution of the remaining payment amount is handled based on the [fee_handling](./create-private.md) configuration.
...
You need to go through [setting up a validator node ](#step-1-setting-up-a-validator-node) guide before starting this section.
