...
3.  You have previously [sent deploys](../../developers/cli/sending-transactions.md) to a Casper network
...
Account configuration on a Casper blockchain is stored in a [Merkle Tree](../../concepts/glossary/M.md#merkle-tree) and is a snapshot of the blockchain's [Global State](../../concepts/global-state.md#global-state-head). The representation of global state for a given block can be computed by executing the deploys (including transfers) within the block and its ancestors. The root node of the Merkle Tree identifying a particular state is called the `state-root-hash` and is stored in every executed block.
